    ...
    while (getline(inFile, line)) {
        ...
        if (line == "GATES") {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (s_type == "AND2") {
                    ...
                    std::string s_output;
                    getline(ss, s_output, ',');
                    m_gates.push_back(new And2Gate(m_wires[stoi(s_in1)],
                                                   m_wires[stoi(s_in2)],
                                                   m_wires[stoi(s_output)]));
                }
                if (s_type == "OR2") {
                    std::string s_in1;
                    getline(ss, s_in1, ',');
                    ...
                    std::string s_output;
                    getline(ss, s_output, ',');
                    m_gates.push_back(new Or2Gate(m_wires[stoi(s_in1)],
                                                  m_wires[stoi(s_in2)],
                                                  m_wires[stoi(s_output)]));
                }
                ...
                if (s_type == "NOT") {
                    ...
                    std::string s_output;
                    getline(ss, s_output, ',');
                    m_gates.push_back(new NotGate(m_wires[stoi(s_in)],
                                                  m_wires[stoi(s_output)]));
                }
            }
        }
        if (line == "INJECT") {
            ...
            for (int i = 0; i < n; i++) {
                ...
                std::string s_state;
                getline(ss, s_state, ',');
                Event *e = new Event{static_cast<uint64_t>(stoi(s_time)),
                                     m_wires[stoi(s_wire)], s_state[0]};
                // std::cout << s_time << "," << s_wire << "," << s_state <<
                // std::endl;
                m_pq.push(e);
            }
        }
...
bool Circuit::advance(std::ostream &os) {
    ...
    while (m_pq.top()->time == m_current_time) {
        if (m_pq.size() >= 1) {
            std::string temp =
                m_pq.top()->wire->setState(m_pq.top()->state, m_current_time);
            if (temp != "") {
                ss << temp << std::endl;
                updated = true;
            }
...
void Circuit::endUml(std::ostream &os) { os << "@enduml" << std::endl; }
